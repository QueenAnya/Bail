...
// ─────────────────────────────────────────────────────────────────────────────
// SECTION 5 — Message Extras Addon (socket-level helpers)
// Ported from innovatorssoft/Baileys
// Adds: profilePictureUrl (incl. newsletter), getEphemeralGroup
...
import {
	getBinaryNodeChild,
	isJidGroup,
	isJidNewsletter as _isJidNewsletter2,
	jidNormalizedUser as _jidNormalizedUser2,
	S_WHATSAPP_NET as _S_WHATSAPP_NET2
} from '../WABinary'
import type { BinaryNode as _BinaryNode2 } from '../WABinary'

export interface MessageExtrasContext {
	query: (node: _BinaryNode2) => Promise<_BinaryNode2>
	/**
...
export const makeMessageExtrasAddon = (ctx: MessageExtrasContext) => {
	...
	const profilePictureUrl = async (jid: string): Promise<string | null> => {
		if (_isJidNewsletter2(jid) && newsletterWMexQuery) {
		...
		const result = await query({
			tag: 'iq',
			attrs: {
				target: _jidNormalizedUser2(jid),
				to: _S_WHATSAPP_NET2,
				type: 'get',
