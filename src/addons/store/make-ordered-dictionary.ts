/**
 * Generic ordered, ID-indexed collection used by the in-memory store to
 * keep each chat's messages both in insertion order (`array`) and
 * accessible by ID in O(1) (`dict`).
 *
...
export function makeOrderedDictionary<T>(idGetter: (item: T) => string): OrderedDictionary<T> {
	...
	return {
		...
		update,
		remove,
		updateAssign: (id: string, partial: Partial<T>): boolean => {
			const item = get(id)
			if (item) {
