 ...
 * (headings, ==highlight==, _italics_, **bold**, etc. render as WhatsApp's own
 * AI-message markdown, not plain text).
 */
export const sendMarkdown = (markdownText: string, quoted?: proto.IWebMessageInfo) => {
	const submessages: RichSubMessage[] = [{ messageType: RichSubMessageType.TEXT, messageText: markdownText }]
	const ctx = buildRichContextInfo(quoted)
	return {
		message: buildBotForwardedMessage(submessages, ctx),
		messageId: generateMessageIDV2()
