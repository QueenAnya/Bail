...
export class TemplateManager {
	...
	create(options: { id?: string; name: string; content: string; description?: string; category?: string }): MessageTemplate {
	...
	getByName(name: string) { return Array.from(this.templates.values()).find(t => t.name === name) }
	getAll() { return Array.from(this.templates.values()) }
	getByCategory(category: string) { return Array.from(this.templates.values()).filter(t => t.category === category) }
	...
	delete(id: string) { return this.templates.delete(id) }
	...
	export(): string { return JSON.stringify(Array.from(this.templates.values()), null, 2) }
...
export const PRESET_TEMPLATES = {
	ORDER_CONFIRMATION: {
		name: 'Order Confirmation', category: 'order',
	...
	REMINDER: {
		name: 'Reminder', category: 'notification',
	...
	BIRTHDAY: {
		name: 'Birthday Wishes', category: 'greeting',
