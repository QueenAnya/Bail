...
export type SearchableMessageType =
	| 'text'
	| 'image'
	| 'video'
	| 'document'
...
export interface SearchOptions {
	...
	toDate?: Date
	limit?: number
	messageTypes?: SearchableMessageType[]
...
const getMessageType = (message: WAMessage): SearchableMessageType | 'other' => {
	const content = message.message
...
export const searchMessages = (messages: WAMessage[], query: string, options: SearchOptions = {}): SearchResult[] => {
	...
	for (const message of messages) {
		...
		if (options.messageTypes && options.messageTypes.length > 0) {
			...
			if (!options.messageTypes.includes(type as SearchableMessageType)) continue
		}
...
export class MessageSearchManager {
	...
	getByType(type: SearchableMessageType): WAMessage[] {
