...
import {
	...
	defaultCompanionPlatformDisplay,
...
describe('defaultCompanionPlatformDisplay', () => {
	it('formats as "browser (os)"', () => {
		expect(defaultCompanionPlatformDisplay(['Windows', 'Chrome', '10.0'])).toBe('Chrome (Windows)')
	})

	it('reflects a custom browser[0] product name as-is', () => {
		expect(defaultCompanionPlatformDisplay(['My Product', 'Chrome', '10.0'])).toBe('Chrome (My Product)')
	})
})
