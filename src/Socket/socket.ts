...
export const makeSocket = (config: SocketConfig) => {
	...
	*/
	...
	const onWhatsApp = async (...phoneNumber: string[]) => {
		...
		const lidResults: { jid: string; exists: boolean; lid: string | undefined }[] = []
		...
		if (results) {
			...
			const rekeyedPnResults = pnResults.map(r => {
				...
				return originalLid ? { ...r, jid: originalLid, lid: originalLid } : r
			})

			return rekeyedPnResults.concat(lidResults)
		}

		return lidResults.length > 0 ? lidResults : undefined
	}
	...
	const sendPairingIQ = async (phoneNumber: string, customPairingCode?: string): Promise<string> => {
		...
		const pairingPlatformHost = getPairingCodeOsDisplay(browser[0]) // custom browser[0] falls back to 'Mac OS'
