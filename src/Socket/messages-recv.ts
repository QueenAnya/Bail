...
import type { WAInitiateCallOptions, WAInitiateCallResult } from '../Types/Call'
import { ReachoutTimelockEnforcementType, WAMessageStatus, WAMessageStubType } from '../Types'
import type { NewChatMessageCapInfo } from '../Types/State'
import type { WAInitiateCallOptions, WAInitiateCallResult } from '../Types/Call'
import {
	aesDecryptCTR,
	aesEncryptGCM,
...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const handleMexNotification = async (node: BinaryNode) => {
		...
		if (updateNode) {
			...
			if (!opName) { await handleLegacyMexNewsletterNotification(node); return }
			let mexResponse: MexGqlResponse
			try { mexResponse = JSON.parse(updateNode.content!.toString()) }
			catch { await handleLegacyMexNewsletterNotification(node); return }
			const data = mexResponse.data
			if (!data) { return }
			switch (opName) {
				case 'NotificationUserReachoutTimelockUpdate': handleReachoutTimelockNotification(data); break
				case 'MessageCappingInfoNotification': handleMessageCappingNotification(data); break
				default: await handleLegacyMexNewsletterNotification(node); break
			}
			return
		}
	...
	const handleReachoutTimelockNotification = (data: MexGqlData) => {
		...
		ev.emit('connection.update', {
			reachoutTimeLock: {
				...
				enforcementType: (payload.enforcement_type as ReachoutTimelockEnforcementType) || ReachoutTimelockEnforcementType.DEFAULT,
				timeEnforcementEnds: payload.time_enforcement_ends ? new Date(parseInt(payload.time_enforcement_ends, 10) * 1000) : undefined
			}
		})
	}
