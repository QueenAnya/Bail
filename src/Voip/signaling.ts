...
export type BaileysSocket = {
	...
	sendNode: (node: any) => Promise<void> | void
	...
	/** Reuses the existing 'set' privacy-tokens IQ — its response already
	 *  contains the freshly-issued token, same shape baileys-caller's
	 *  standalone `getPrivacyTokens` expected from a dedicated 'get'. */
...
export class SignalingBridge {
	...
	requestTcToken = async (jid: string): Promise<Uint8Array | undefined> => {
		...
		try {
			const response = await this.#sock.issuePrivacyTokens([userJid])
			const tokensNode =
	...
	discoverPeerDevices = async (peerLidJid: string): Promise<string[]> => {
		...
		const devices = await this.#sock.getUSyncDevices([peerLidJid], true, true)
	...
	#toCallDeviceJid = (jid: string): string => {
		...
		if (!decoded?.user) return jid
		const server = jid.endsWith('@lid') ? 'lid' : 's.whatsapp.net'
		if (decoded.device === null || decoded.device === undefined) return jidEncode(decoded.user, server)
	...
	#toPrimaryDeviceJid = (jid: string): string | undefined => {
		...
		if (device === null || device === undefined || device === 0) return undefined
		const server = jid.endsWith('@lid') ? 'lid' : 's.whatsapp.net'
		return jidEncode(decoded.user, server)
	}
	...
	#hasConcreteDevice = (jid: string): boolean => {
		const decoded = jidDecode(jid)
		return !!decoded?.user && decoded.device !== null && decoded.device !== undefined
	}
	...
	#normalizeStartCallPeerList = (jids: string[]): string[] => {
		...
		for (const jid of jids) {
			...
			if (decoded.device !== null && decoded.device !== undefined) {
