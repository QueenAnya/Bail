...
export class RelayRtcTransport {
	...
	updateRelayList = (update: RelayListUpdatePayload): void => {
		...
		for (const relay of update.relays ?? []) {
			...
			const authToken =
				update.auth_tokens &&
				relay.auth_token_id !== null &&
				relay.auth_token_id !== undefined &&
				relay.auth_token_id >= 0
			...
			for (const address of relay.addresses ?? []) {
				...
				if (address.ipv4 && address.port !== null && address.port !== undefined) {
				...
				if (!DISABLE_IPV6 && address.ipv6 && address.port_v6 !== null && address.port_v6 !== undefined) {
	...
	#startIceRttPolling = (connection: RelayConnectionRuntime): void => {
		...
		const poll = async (): Promise<void> => {
			...
			if (rttMs === null || rttMs === undefined || connection.lastIceRttMs === rttMs) return
			...
			const rttSeconds =
				typeof selectedPair?.currentRoundTripTime === 'number'
					...
					: typeof selectedPair?.totalRoundTripTime === 'number' &&
						...
						: null
			if (rttSeconds === null || rttSeconds === undefined || !Number.isFinite(rttSeconds) || rttSeconds <= 0)
				return null
