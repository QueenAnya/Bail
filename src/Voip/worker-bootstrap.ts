...
if (typeof process === 'undefined') {
	...
	;(process as any).cwd = () => __voipDirname || '.'
}
...
;(global as any).babelHelpers = {
	...
	objectSpread: function (target: any) {
		for (var i = 1; i < arguments.length; i++) {
			var source = arguments[i] !== null && arguments[i] !== undefined ? arguments[i] : {}
...
const nullthrows = <T>(value: T | null | undefined): T => {
	if (value === null || value === undefined) throw new Error('Got unexpected null or undefined')
	return value
}
...
if (!wasmLoader) {
	const resourcesPath = typedWorkerData?.resourcesPath || path.join(__voipDirname, 'wasm-resources')
