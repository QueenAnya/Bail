...
const __voipFilename = fileURLToPath(import.meta.url)
const __voipDirname = path.dirname(__voipFilename)
...
const resolveWorkerScriptPath = (): string => {
	const compiled = path.join(__voipDirname, 'worker-bootstrap.js')
	return fs.existsSync(compiled) ? compiled : path.join(__voipDirname, 'worker-bootstrap.ts')
}
...
export class WasmEngine {
	...
	constructor(config: WasmEngineConfig = {}) {
		...
		// correctly reached the package root. queenanya's build mirrors `src/`'s
	...
	#requireModule = (name: string): any => {
		...
		const importAllFn = (dep: string): any => {
			const v = requireFn(dep)
			if (v === null || v === undefined) return { default: v }
		...
		if (
			(metroError !== null && metroError !== undefined) ||
