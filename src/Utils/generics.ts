...
export const printQRIfNecessaryListener = (ev: import('../Types').BaileysEventEmitter, logger: ILogger) => {
	ev.on('connection.update', async ({ qr }) => {
		if (qr) {
			const QR = await import('qrcode-terminal')
				.then(m => (m as any).default || m)
				.catch(() => {
					logger.warn('install qrcode-terminal for automatic QR display')
					return null
				})
