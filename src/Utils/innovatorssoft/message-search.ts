...
export type MessageType = 'text' | 'image' | 'video' | 'document' | 'audio' | 'sticker' | 'location' | 'contact' | 'other'
...
export const searchMessagesRegex = (messages: WAMessage[], pattern: RegExp, options: Omit<SearchOptions, 'caseSensitive'> = {}): SearchResult[] => {
	const results: SearchResult[] = []
	for(const message of messages) {
		if(options.jid && message.key.remoteJid !== options.jid) continue
		if(options.fromSender && message.key.participant !== options.fromSender) continue
...
export class MessageSearchManager {
	...
	clear() { this.messages = []; this.messageIndex.clear() }
	get count() { return this.messages.length }
	search(query: string, options?: SearchOptions) { return searchMessages(this.messages, query, options) }
	searchRegex(pattern: RegExp, options?: Omit<SearchOptions, 'caseSensitive'>) { return searchMessagesRegex(this.messages, pattern, options) }
	getByJid(jid: string) { return this.messages.filter(m => m.key.remoteJid === jid) }
	getBySender(sender: string) { return this.messages.filter(m => m.key.participant === sender || m.key.remoteJid === sender) }
	getByType(type: MessageType) { return this.messages.filter(m => getMessageType(m) === type) }
	getById(id: string) { return this.messageIndex.get(id) }
}
