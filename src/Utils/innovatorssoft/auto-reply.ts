...
export class AutoReplyHandler {
	...
	addRule(rule: AutoReplyRule) {
		const fullRule = { ...rule, id: rule.id ?? this.generateId(), active: rule.active ?? true, priority: rule.priority ?? 0 }
	...
	removeRule(id: string) { return this.rules.delete(id) }
	...
	setRuleActive(id: string, active: boolean) {
		...
		if(r) { r.active = active; return true }
		return false
	}
